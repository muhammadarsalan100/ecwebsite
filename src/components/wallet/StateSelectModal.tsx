...
import { State, StateSelectModalProps } from "@/types";
import { useConfigStore } from "@/lib/store/configStore";

export function StateSelectModal({ isOpen, onClose, onSelect, selectedState: propSelectedState, countryId }: StateSelectModalProps) {
    const { states, fetchStates, isStatesLoading } = useConfigStore();
                ...
                >
                    ...
                    <div className="p-8 pb-4">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-2xl font-bold text-gray-900">Select State</h2>
                    ...
                    <div className="flex-1 overflow-y-auto px-4 pb-8 custom-scrollbar">
                        {!countryId ? (
                            <div className="flex flex-col items-center justify-center h-full text-center px-10">
                                ...
                                <p className="text-gray-500 font-medium">Please select a country first</p>
                            </div>
                        ) : isStatesLoading ? (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={cn(
                                                ...
                                                propSelectedState === state.name
                                                    ? "bg-blue-50 border-blue-200 text-blue-600"
                                                    : "bg-gray-50 border-gray-100 text-gray-400"
                                            )}>
                                                <MapPin className="w-5 h-5" />
                                            </div>
                                            <span className={`text-[16px] font-bold ${propSelectedState === state.name ? 'text-blue-600' : 'text-gray-900'}`}>
                                                {state.name}
                                            </span>
                                        </div>
                                        {propSelectedState === state.name && (
